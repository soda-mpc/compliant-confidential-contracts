...
contract PrivateERC20Contract {
    ...
    gtUint256 public immutable zero;
    ...
    mapping(address => mapping(address => gtUint256)) private allowances;

    /// @notice Deploys the token, assigning the entire initial supply to the deployer
    /// @param name_ The name of the token
    /// @param symbol_ The symbol of the token
    /// @param initialSupply The total number of tokens minted to the deployer
    constructor(string memory name_, string memory symbol_, uint256 initialSupply) {
        _name = name_;
        _symbol = symbol_;
        _totalSupply = initialSupply;
        balances[msg.sender] = MpcCore.setPublic256(initialSupply);
        // Permit the contract and the user to use the balance handle
        MpcCore.permitThis(balances[msg.sender]);
        MpcCore.permit(balances[msg.sender], msg.sender);
    ...
    /// @return The handle to the transfer's result
    function contractTransfer(address _to, gtUint256 _value) public returns (gtBool) {
    ...
    /// @param _it The encrypted and signed transfer amount
    /// @return The handle to the transfer's result
    function transferFrom(address _from, address _to, itUint256 calldata _it) public returns (gtBool) {
        gtUint256 value = MpcCore.validateCiphertext(_it);
    ...
    /// @param _value The amount of tokens to transfer
    /// @return The handle to the transfer's result
    function transferFrom(address _from, address _to, uint256 _value) public returns (gtBool) {
    ...
    /// @return The handle to the transfer's result
    function contractTransferFrom(address _from, address _to, gtUint256 _value) public returns (gtBool) {
        // Check if the sender is permitted to use the _value handle
        require(MpcCore.isSenderPermitted(_value));
    ...
    function approve(address _spender, itUint256 calldata _it) public returns (bool) {
    ...
    /// @return True if the approval was successful
    function contractApprove(address _spender, gtUint256 _value) public returns (bool) {
        // Check if the sender is permitted to use the _value handle
        require(MpcCore.isSenderPermitted(_value));
    ...
    function _getBalances(address _from, address _to) private view returns (gtUint256, gtUint256) {
        return (balanceOf(_from), balanceOf(_to));
    }
