...
contract PrivateERC20Contract is DecryptionCaller {
    ...
    IERC20 public underlying; // The standard ERC20 token that can be wrapped
    ...
    /// @notice Deploys the token, assigning the contract owner and setting the underlying ERC20 token
    /// @param name_ The name of the token
    /// @param symbol_ The symbol of the token
    /// @param underlying_ The address of the underlying ERC20 token
    /// @param owner_ The address that will own the token
    constructor(string memory name_, string memory symbol_, address underlying_, address owner_) {
        _name = name_;
        _symbol = symbol_;
        owner = owner_;
    ...
    function transfer(address _to, itUint64 calldata _it) public virtual returns (gtBool) {
    ...
    /// @return The handle to the transfer's result
    function transfer(address _to, uint64 _value) public virtual returns (gtBool) {
    ...
    /// @return The handle to the transfer's result
    function contractTransfer(address _to, gtUint64 _value) public virtual returns (gtBool) {
    ...
    /// @param _it The encrypted and signed transfer amount
    /// @return The handle to the transfer's result
    function transferFrom(address _from, address _to, itUint64 calldata _it) public virtual returns (gtBool) {
        gtUint64 value = MpcCore.validateCiphertext(_it);
    ...
    /// @param _value The amount of tokens to transfer
    /// @return The handle to the transfer's result
    function transferFrom(address _from, address _to, uint64 _value) public virtual returns (gtBool) {
    ...
    /// @return The handle to the transfer's result
    function contractTransferFrom(address _from, address _to, gtUint64 _value) public virtual returns (gtBool) {
        // Check if the sender is permitted to use the _value handle
        require(MpcCore.isSenderPermitted(_value));
        gtUint64 allowance = _getGTAllowance(_from, msg.sender);
        // Check for self-transfer
    ...
    function approve(address _spender, itUint64 calldata _it) public virtual returns (bool) {
    ...
    /// @return True if the approval was successful
    function approve(address _spender, uint64 _value) public virtual returns (bool) {
    ...
    /// @return True if the approval was successful
    function contractApprove(address _spender, gtUint64 _value) public virtual returns (bool) {
        // Check if the sender is permitted to use the _value handle
        require(MpcCore.isSenderPermitted(_value));
        address owner = msg.sender;
    ...
    function shield(uint256 amount) public virtual returns (bool) {
