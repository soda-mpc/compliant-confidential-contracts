...
contract TransferAllowanceScalar128TestsContract is DecryptionCaller {
    ...
    function setDecryption(gtUint128 newA_s, gtUint128 newB_s, gtBool res_s, gtUint128 newAllowance_s, uint256[] memory arrToDecrypt, uint32 index) internal {
    ...
    function computeAndCheckTransfer128(AllGTCastingValues memory allGTCastingValues, AllAllowanceValues memory allAllowanceValues, uint8 amount, uint256[] memory arrToDecrypt) public {
    ...
    function transferWithAllowanceTest(uint8 a, uint8 b, uint8 amount, uint8 allowance) public returns (uint256) {
        ...
        uint256[] memory arrToDecrypt = new uint256[](4 * (9 * 5));  // 180
        
        // Calculate the result with casting to 128
        computeAndCheckTransfer128(allGTCastingValues, allAllowanceValues, amount, arrToDecrypt);
