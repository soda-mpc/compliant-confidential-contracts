...
export class SodaWeb3Helper {
    constructor(privateKey, httpProviderUrl, chainId = DEFAULT_CHAIN_ID) {
        ...
        // isListening() is async, so the previous `if (!...)` test read a
        // pending Promise, which is always truthy: an unreachable node was
        // never reported here and surfaced later as an unrelated error.
        this.web3.eth.net
            .isListening()
            .then(() => console.log(`Connected to the node at ${httpProviderUrl}`))
            .catch(() => {
                console.error(
                    `Could not reach the Bubble RPC endpoint at ${httpProviderUrl}.\n` +
    ...
    setContractAddress(contractId, contractAddress) {
        if (!(contractId in this.contracts)) {
            throw new Error(`Contract with id ${contractId} does not exist. Use the 'setupContract' method to set it up.`);
        }
        
        this.contracts[contractId].options.address = contractAddress;
    }

    getContractAddress(contractId) {
        if (!(contractId in this.contracts)) {
            throw new Error(`Contract with id ${contractId} does not exist. Use the 'setupContract' method to set it up.`);
        }
        
        return this.contracts[contractId].options.address;
    }
    ...
    async callContractTransaction(contractId, funcName, funcArgs = [], account = this.account, gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE) {
        if (!(contractId in this.contracts)) {
            console.log(`Contract with id ${contractId} does not exist. Use the 'setupContract' method to set it up.`);
            return null;
        }
        ...
        if (!contractAddress) {
            throw new Error(`Contract address not set for ${contractId}. Make sure the contract is deployed or address is set.`);
        }
        
        const funcToCall = contract.methods[funcName](...funcArgs);
        const receipt = await this.callContractFunctionTransaction(funcToCall, account, contractAddress, gasLimit, gasPrice);
        return receipt;
    }

    async callContractFunctionTransaction(func, account = this.account, contractAddress = null, gasLimit = DEFAULT_GAS_LIMIT, gasPrice = DEFAULT_GAS_PRICE) {
        // If contractAddress is not provided, try to get it from func._parent
        if (!contractAddress) {
            if (func._parent && func._parent.options && func._parent.options.address) {
                contractAddress = func._parent.options.address;
            } else {
                throw new Error('Contract address not set. Make sure the contract is deployed or address is set.');
            }
        }
        
        const nonce = await this.web3.eth.getTransactionCount(account.address);
        const encodedData = func.encodeABI();
        ...
        // Sign and send the transaction manually if using a different account
        if (account.address !== this.account.address || account.privateKey !== this.account.privateKey) {
            const receipt = await this.signAndSendTransactionWithKey(transaction, account.privateKey);
            return receipt;
        } else {
            // Use the default sendTransaction method for the default account
            const receipt = await this.sendTransaction(func, transaction);
            return receipt;
        }
    }
    
    async signAndSendTransactionWithKey(transaction, privateKey) {
        try {
            ...
            console.error('Error sign and send transaction with key:', error);
            throw error;
        }
    }
