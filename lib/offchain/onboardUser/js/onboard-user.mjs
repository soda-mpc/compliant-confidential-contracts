import fs from 'fs';
import { generateRSAKeyPair, decryptRSA, sign, verifySignatures } from '../../../soda-sdk/js/crypto.mjs';
import { privateToAddress } from 'ethereumjs-util';
import { createUserInteractorClient } from '../../../onchain/scripts/js/soda-web3-helper.mjs';
import { getSignersAddresses } from '../../../onchain/scripts/js/get_signers.mjs';
...
async function onboard_user(client, signingKey, signers) {
    ...
    // Validate signatures 
    if (!verifySignatures(response.rsa_ciphertexts, response.mpc_signatures, signers)){
    ...
    const share0 = decryptRSA(privateKey, cipher0);
    const share1 = decryptRSA(privateKey, cipher1);
...
async function main() {    
    ...
    const user_key = await onboard_user(client, signingKey, signers);
